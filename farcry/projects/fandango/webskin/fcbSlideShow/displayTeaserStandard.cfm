...
<cfif NOT structKeyExists(session, 'dmSec') 
	...
	<cfif arrayLen(stObj.aObjectIDs)>
		...
		<cfoutput>			
			<link type="text/css" rel="stylesheet" href="#application.url.webroot#/js/galleria/themes/classic/galleria.classic.css">
			<div class="bannerRotator">
				<script>
				window.onload = function() {

			    	var imageData = #SerializeJSON(aImageData)#;				    	
				 	Galleria.loadTheme('#application.url.webroot#/js/galleria/themes/dots/galleria.dots.js');
				    Galleria.run('.banner',
				    {
				    	responsive:true,
				    	lightbox: false,
...
<cfelse>
	<cfoutput><p><i>Slider is hidden when logged in to farcry.</i></p></cfoutput>
</cfif>
